...
def get_feature_selection_inputs(feature_importance_df, feature_names):
    ...
    with col1:
        ...
        # Number of intervals for first feature
        num_intervals1 = st.slider(
            f'Number of Intervals for {selected_feature1}', 
            min_value=3, 
            max_value=10, 
            value=5,
            key='joint_importance_intervals1'
        )

    with col2:
        # Select second feature
        remaining_features = [f for f in feature_names if f != selected_feature1]
        ...
        # Number of intervals for second feature
        num_intervals2 = st.slider(
            f'Number of Intervals for {selected_feature2}', 
            min_value=3, 
            max_value=10, 
            value=5,
            key='joint_importance_intervals2'
        )
    
    return selected_feature1, selected_feature2, feature1_index, feature2_index, num_intervals1, num_intervals2

def _define_joint_intervals(X_test, feature_1_index=0, feature_2_index=1, num_intervals=5):
    """
    Define joint intervals for two features.
    """
    # Get min and max values for both features
    min_val_1 = X_test.iloc[:, feature_1_index].min()
    max_val_1 = X_test.iloc[:, feature_1_index].max()
    min_val_2 = X_test.iloc[:, feature_2_index].min()
    max_val_2 = X_test.iloc[:, feature_2_index].max()

    # Define intervals for both features
    intervals_1 = np.linspace(min_val_1, max_val_1, num_intervals + 1)
    intervals_2 = np.linspace(min_val_2, max_val_2, num_intervals + 1)
...
def visualize_joint_importance(model, X_test, y_test, feature_1_index=0, feature_2_index=1,
                             num_intervals1=5, num_intervals2=5):
    """
    ...
    for metric_name in selected_metrics:
        ...
        fig = go.Figure(data=go.Heatmap(
            ...
            colorscale='RdYlBu_r',
            zmin=0,
            zmax=max(np.max(metric_values), 0.001),
            colorbar=dict(
